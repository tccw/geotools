...
cana_vel = np.load(r'data\coal_well_vels_ft_s_m_depth_0_152399_7m_backus.npy') * 0.3048
cana_vel_clip = cana_vel[:,:]
...
plt.scatter(x_rec,y_rec, color = 'b', marker = 'v', s=90)
plt.scatter(x_src,y_src, color = 'r', marker = 'D', s=15)
plt.ylim(area[2:][::-1])
...
for i in range(len(seismogram_list)):
    ...
    plt.plot(plot_spacing*i, 0, 'vb', markersize=10, linewidth=1)
...
anim.save('cana_full_res_mistake.mp4', fps=30, dpi=200, bitrate=4000)
